// dsf_07_lantern_bitmask - sparse twin pulses behind bitfield thresholds
(async () => {
  ...
  let pulse = rnd_btw(0.22, 0.62);
  let cells = rnd_int(18, 44);
  let p = rnd_int(30, 72);

  osc(pulse, 0.01, 0.2).thresh(() => 0.78 - a.fft[0] * 0.42).scrollX(-0.2)
    .add(osc(pulse * 1.15, 0.01, 0.2).thresh(() => 0.78 - a.fft[1] * 0.4).scrollX(0.2), 0.95)
    .mult(osc(rnd_btw(50, 130), 0, 1).thresh(() => 0.5 - a.fft[2] * 0.18).pixelate(cells, cells).repeat(rnd_int(2, 5), 1))
    .modulateRepeat(noise(2, 0.35).pixelate(3, 3), () => 0.03 + a.fft[1] * 0.18, rnd_int(4, 10))
    .pixelate(p, p)
    .add(src(o0).scale(1.008).luma(0.05).colorama(0.018), 0.6)
    .add(noise(9, 0.25).thresh(() => 0.95 - a.fft[3] * 0.2).pixelate(p * 2, p * 2), 0.38)
